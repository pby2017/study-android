...
Current position: continue after `AI 랩이 개발 툴체인을 사들이는 중 — Anthropic→Bun, OpenAI→Astral(uv·ruff)`; next oldest reviewed candidate is `ccpi + tonsofskills.com — 플러그인·스킬·에이전트를 키워드로 검색·설치하는 오픈소스 마켓플레이스 CLI`.

Last update: 2026-06-26 00:29 KST
...
Next:
- Continue with `ccpi + tonsofskills.com — 플러그인·스킬·에이전트를 키워드로 검색·설치하는 오픈소스 마켓플레이스 CLI`; refine only if current public sources support clearer detail than the existing card.
